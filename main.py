...
async def get_report(session_id: str):
    ...
    Generates on first call, cached for session lifetime.
    Returns 202 if interview is not yet complete.
    Phase 5 will replace the stub report with report_generator.py.
    """
    with _get_db() as db:
        session = _get_session(db, session_id)
        if session is None:
        ...
        # Stub report — Phase 5 will call report_generator.generate_report() here
        report = {
            "session_id": session_id,
            "agent_name": session["agent_name"],
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "culture_signal_used": session["culture_signal"] is not None,
            "dimension_scores": {
                "communication_style": {"score": 0, "max": 5, "reasoning": "Pending — report generator not yet implemented"},
                "decision_autonomy": {"score": 0, "max": 5, "reasoning": "Pending"},
                "escalation_threshold": {"score": 0, "max": 5, "reasoning": "Pending"},
                "risk_tolerance": {"score": 0, "max": 5, "reasoning": "Pending"},
                "ambiguity_handling": {"score": 0, "max": 5, "reasoning": "Pending"},
                "values_under_pressure": {"score": 0, "max": 5, "reasoning": "Pending"},
            },
            "risk_flags": [],
            "recommendations": ["Report generator not yet implemented — scores are placeholders."],
            "suggested_system_prompt": "Pending — will be generated by report_generator.py in Phase 5.",
        }
